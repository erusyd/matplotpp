...
void MatPlot::display()
{
    ...
    while (!is_done) {
        ...
        if (mode == 1) { // Configuration of objects
            ...
            // checking number of objects. if it is inconsistent, then objects
            // are reallocated.
            if (vFigure.size() != iFigure) {
                mode_next = 0;
                if (is_debug1) {
                    cout << "not match in Figure" << endl;
                }
            }
            if (vAxes.size() != iAxes) {
                mode_next = 0;
                if (is_debug1) {
                    cout << "not match in Axes" << endl;
                }
            }
            if (vLine.size() != iLine) {
                mode_next = 0;
                if (is_debug1) {
                    cout << "not match in Line" << endl;
                }
            }
            if (vSurface.size() != iSurface) {
                mode_next = 0;
                if (is_debug1) {
                    cout << "not match in Surface" << endl;
                }
            }
            if (vText.size() != iText) {
                mode_next = 0;
                if (is_debug1) {
                    cout << "not match in Text" << endl;
                }
            }
            if (vLayer.size() != iLayer) {
                mode_next = 0;
                if (is_debug1) {
                    cout << "not match in Layer" << endl;
                }
            }
            if (vColorbar.size() != iColorbar) {
                mode_next = 0;
                if (is_debug1) {
                    cout << "not match in Colorbar" << endl;
                }
            }
...
float MatPlot::ctx2(double x)
{
    ...
    if (ca->XScale == 0) { // linear
        return ca->Position[0] +
               ca->Position[2] *
                   ((x - ca->XLim[0]) / (ca->XLim[1] - ca->XLim[0]));
    }
    if (ca->XScale == 1) { // log
        t = (log10(x) - log10(ca->XLim[0])) /
            (log10(ca->XLim[1]) - log10(ca->XLim[0]));
        if (x <= 0) {
            t = -1;
        }
...
float MatPlot::cty2(double y)
{
    if (ca->YScale == 0) { // linear
        return ca->Position[1] +
               ca->Position[3] *
                   ((y - ca->YLim[0]) / (ca->YLim[1] - ca->YLim[0]));
    }
    if (ca->YScale == 1) { // log
        return ca->Position[1] + ca->Position[3] *
                                     (log10(y) - log10(ca->YLim[0])) /
                                     (log10(ca->YLim[1]) - log10(ca->YLim[0]));
    }
}
...
float MatPlot::ctx(double x)
{
    ...
    if (ca->XScale == 1) { // log
        return (log10(x) - log10(ca->XLim[0])) /
               (log10(ca->XLim[1]) - log10(ca->XLim[0]));
    }
}
...
float MatPlot::cty(double y)
{
    ...
    if (ca->YScale == 1) { // log
        return (log10(y) - log10(ca->YLim[0])) /
               (log10(ca->YLim[1]) - log10(ca->YLim[0]));
    }
}
...
void MatPlot::set(int h, string p, string v)
{
    ...
    if ((tObj == tLine) && (iObj < vLine.size())) {
        ...
        if (p == "LineStyle") {
            vLine[iObj].LineStyle = v;
        }

        if (p == "MarkerEdgeColor") {
            vLine[iObj].MarkerEdgeColor = v;
        }
        if (p == "MarkerFaceColor") {
            vLine[iObj].MarkerFaceColor = v;
        }
        // if(p==""){ vLine[iObj].=v; }
    }
    // Surface //
    if ((tObj == tSurface) && (iObj < vSurface.size())) {
        if (p == "COLOR") {
            vSurface[iObj].EdgeColor = v;
        }
        if (p == "LineStyle") {
            vSurface[iObj].LineStyle = v;
        }
        if (p == "EdgeColor") {
            vSurface[iObj].EdgeColor = v;
        }
        if (p == "FaceColor") {
            vSurface[iObj].FaceColor = v;
        }
        // if(p==""){ vLine[iObj].=v; }
    }
    // Patch //
    if ((tObj == tPatch) && (iObj < vPatch.size())) {
        // if(p==""){ vPatch[iObj].=v; }
        if (p == "COLOR") {
            vPatch[iObj].EdgeColor = v;
        }
        if (p == "LineStyle") {
            vPatch[iObj].LineStyle = v;
        }
        if (p == "EdgeColor") {
            vPatch[iObj].EdgeColor = v;
        }
        if (p == "FaceColor") {
            vPatch[iObj].FaceColor = v;
        }
    }
}
...
void MatPlot::set(int h, string p, float v)
{
    ...
    if ((tObj == tPatch) && (iObj < vPatch.size())) {
        if (p == "LineWidth") {
            vPatch[iObj].LineWidth = v;
        }
        // if(p==""){ vLine[iObj].=v; }
    }
}

int MatPlot::gco() { return hObj; }
...
void MatPlot::print()
{
    ...
    while (state == GL2PS_OVERFLOW) {
        ...
        gl2psBeginPage("test", "gl2ps", NULL, GL2PS_EPS, GL2PS_SIMPLE_SORT,
                       GL2PS_USE_CURRENT_VIEWPORT, GL_RGBA, 0, NULL, 0, 0, 0,
                       buffsize, fp, "out.eps");
        display();
        state = gl2psEndPage();
    }
...
int MatPlot::figure()
{
    ...
    hObj = h;
    if (is_debug1) {
        printf("mode: %d handle: %4d Figure\n", mode, h);
    }
    if (mode == 0) {
...
{
    if (is_debug1) {
        printf("mode: %d handle: %4d Layer ( %s ) \n", mode, cfr->id,
               cfr->layername.c_str());
    }
    ...
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) { // Left Click
        for (int j = 0; j < vLayer.size(); ++j) {
            if ((l < x) && (x < w) && (h * j < y) && (y < h * j + h)) {
                ...
                dt = (float)(time_layer_clicked - time_layer_clicked_last) /
                     CLOCKS_PER_SEC;
                if (dt < 0.05) {
                    for (int k = 0; k < vLayer.size(); ++k) {
                        vLayer[k].Visible = 0;
                    }
                    vLayer[j].Visible = 1;
                    // cout <<"!!"<<endl;
                }
                // cout <<"click: "<<time_layer_clicked<<" "<<CLOCKS_PER_SEC<<"
                // "<<dt<<endl;
                time_layer_clicked_last = time_layer_clicked;

                // cout<<"pushed:"<<j<<" Visible:"<<vLayer[j].Visible<<"
                // "<<is<<endl;
                for (int i = 0; i < vAxes.size(); ++i) {
                    if (vAxes[i].Parent = vLayer[j].id) {
                        vAxes[i].Visible = is;
                    }
                }
            }
...
{
    if (is_debug1) {
        printf("mode: %d handle: %4d Axes  #Children %d\n", mode, ca->id,
               ca->Children.size());
    }
...
void MatPlot::display_axes_2d()
{
    ...
    if (ca->Box) {
        ...
        if (ca->TickLabel) {
            for (int i = 0; i < ca->XTick.size(); ++i) {
                ...
                ptext(ctx2(ca->XTick[i]) -
                          (float)num_char * char_w / window_w / 2.0,
    ...
    ptext(l + w / 2.0 - (float)num_char * char_w / window_w / 2.0,
          b - offset - 2.0 * char_h / window_h, ca->XLabel);
    ...
    glViewport(
        (int)(ca->Position[0] * window_w), (int)(ca->Position[1] * window_h),
        (int)(ca->Position[2] * window_w), (int)(ca->Position[3] * window_h));
    glLoadIdentity();
    gluOrtho2D(0.0, 1.0, 0.0, 1.0);
}
...
void MatPlot::display_axes_3d()
{
    ...
    */
    ...
    gluLookAt(
        cos(ca->cta * PI / 180) * cos(ca->phi * PI / 180),
    ...
    glColor3f(0,0,0);
    glBegin(GL_LINE_LOOP);
    glVertex3d(-1,-1,0); glVertex3d(1,-1,0); glVertex3d(1,1,0);
    glVertex3d(-1,1,0); glEnd();
    */
        ...
        for (int i = 0; i < vAxes.size(); ++i) {
            ...
            if (ca->Visible) {
                ...
                if ((l <= X) && (X <= l + w) && (b <= Y) && (Y <= b + h)) {
                    ...
                    // cout << "axes "<< i << " is selected "<<
                    // vAxes[i].selected()  << endl; cout <<"(cta,phi) = "<<
                    // ctaButtonDown <<" "<< phiButtonDown << endl;
                }
            }
        }

        if (is_debug2) {
            cout << "selected axes" << iAxesSelected << endl;
        }
...
void MatPlot::Axes_motion(int x, int y)
{
    ...
    for (int i = 1; i < vAxes.size(); ++i) {
        ...
        if ((ca->Selected) && (ca->View == 1)) {
            ...
            ca->phi = phi;
            ca->cta = cta;
            // cout <<"( phi,cta ) = ( "<< vAxes[i].phi <<","<< vAxes[i].cta <<"
            // )"<<endl;
        }
    }
    ...
    // cout <<"( dx,dy ) = ( "<< x-xButtonDown <<","<<y-yButtonDown <<"
    // )"<<endl; cout <<"( phi,cta ) = ( "<< phi <<","<< cta <<" )"<<endl;
...
void MatPlot::xaxis(double xMin, double xMax)
{ // not in Matlab
    if (xMin != xMax) {
        ca->XLim[0] = xMin;
        ca->XLim[1] = xMax;
...
void MatPlot::yaxis(double yMin, double yMax)
{ // not in Matlab
    if (yMin != yMax) {
        ca->YLim[0] = yMin;
        ca->YLim[1] = yMax;
...
void MatPlot::title(string s) { ca->Title = s; }
void MatPlot::xlabel(string s) { ca->XLabel = s; }
void MatPlot::ylabel(string s) { ca->YLabel = s; }
...
void MatPlot::display_axes_colorbar()
{
    ...
    if (ca->Box) {
        ...
        for (int i = 0; i < ca->CTick.size(); ++i) {
            ...
            glVertex2d(l + w, b + h * ((ca->CTick[i] - ca->CLim[0]) /
                                       (ca->CLim[1] - ca->CLim[0])));
            glVertex2d(l + w + 0.01, b + h * ((ca->CTick[i] - ca->CLim[0]) /
                                              (ca->CLim[1] - ca->CLim[0])));
            glEnd();
        }
        // z tick number
...
int MatPlot::plot(dvec x, dvec y) { return line(x, y); }
...
/// create errorbar
void MatPlot::vertex(double x, double y, double ep, double em)
{ // for errorbar
    if (ca->xmin > x) {
        ca->xmin = x;
    }
        ...
        if (cl->Marker != "none") { // Marker //
            ...
            for (int i = 0; i < cl->XData.size(); ++i) {
                ...
                if (cl->Marker == "o") { // o
                    ...
                    for (int i = 0; i < 20; i++) {
                        glVertex2d(
                            xx + rx * cos(2 * PI * (double)i / (double)(20)),
        ...
        if (cl->LineStyle != "none") { // Line //
            ...
            if (cl->XData.size()) {
                ...
                for (int i = 0; i < cl->XData.size(); ++i) {
                    ...
                    // yy=cty(cl->YData[i]);
                    // glVertex2d(xx,yy);
                    glVertex3d(ct3x(cl->XData[i]), ct3y(cl->YData[i]),
                               ct3z(cl->ZData[i]));
                }
                glEnd();
            }
...
int MatPlot::surface()
{
    int h = iSurface * 100 + tSurface;
    hObj = h;
    if (is_debug1) {
        printf("mode: %d handle: %4d Surface\n", mode, h);
    }
    ...
    if (iSurface < vSurface.size()) {
        cs = &vSurface[iSurface];
    }
    iSurface++;
    return h;
}
...
{
    if (is_debug1) {
        printf("mode: %d handle: %4d Surface type %d, Z.size %d\n", mode,
               cs->id, cs->type, cs->ZData.size());
    }
...
void MatPlot::display_surface_2d()
{
    ...
    // __LINE__,nxi,nxj,nyi,nyj,nzi,nzj);
    ...
    if (nxi == 0) {
        ...
        if (cs->EdgeColor != "none") {
            ...
            for (int i = 0; i < nzi; ++i) {
                ...
                for (int j = 0; j < nzj; ++j) {
                    glVertex2d(ctx(ca->XLim[0] + (ca->XLim[1] - ca->XLim[0]) *
                                                     (float)(j) / (nzj - 1)),
                               cty(ca->YLim[0] + (ca->YLim[1] - ca->YLim[0]) *
                                                     (float)(i) / (nzi - 1)));
                }
                glEnd();
            }
        ...
        if (cs->FaceColor != "none") {
            for (int i = 0; i < nzi - 1; ++i) {
                for (int j = 0; j < nzj - 1; ++j) {
                    ...
                    glBegin(GL_QUADS);
                    glVertex2d(
                        ctx(ca->XLim[0] + (ca->XLim[1] - ca->XLim[0]) *
                                              (float)(j) / (float)(nzj - 1)),
                        cty(ca->YLim[0] + (ca->YLim[1] - ca->YLim[0]) *
                                              (float)(i) / (float)(nzi - 1)));
                    glVertex2d(ctx(ca->XLim[0] + (ca->XLim[1] - ca->XLim[0]) *
        ...
        if (cs->FaceColor != "none") {
            for (int i = 0; i < nyj - 1; ++i) {
                for (int j = 0; j < nxj - 1; ++j) {
                    ...
                    glBegin(GL_QUADS);
                    glVertex2d(ctx(cs->XData[0][j]), cty(cs->YData[0][i]));
                    glVertex2d(ctx(cs->XData[0][j]), cty(cs->YData[0][i + 1]));
                    glVertex2d(ctx(cs->XData[0][j + 1]),
                               cty(cs->YData[0][i + 1]));
                    glVertex2d(ctx(cs->XData[0][j + 1]), cty(cs->YData[0][i]));
                    glEnd();
                }
            }
        }
    ...
    if (nxi > 1) {
        ...
        if (cs->EdgeColor != "none") {
            ...
            for (int i = 0; i < nxi; ++i) {
                glBegin(GL_LINE_STRIP); // TODO add more style
                for (int j = 0; j < nxj; ++j) {
                    glVertex2d(ctx(cs->XData[i][j]), cty(cs->YData[i][j]));
                }
                glEnd();
            }
            for (int j = 0; j < nxi; ++j) {
        ...
        if (cs->FaceColor != "none") {
            for (int i = 0; i < ny - 1; ++i) {
                for (int j = 0; j < nx - 1; ++j) {
                    ...
                    glVertex3d(ct3x(cs->XData[0][j]), ct3y(cs->YData[0][i]),
                               ct3z(cs->ZData[i][j]));
                    glVertex3d(ct3x(cs->XData[0][j]), ct3y(cs->YData[0][i + 1]),
                               ct3z(cs->ZData[i + 1][j]));
                    ...
                    glVertex3d(ct3x(cs->XData[0][j]), ct3y(cs->YData[0][i]),
                               ct3z(cs->ZData[i][j]));
                    glVertex3d(ct3x(cs->XData[0][j + 1]), ct3y(cs->YData[0][i]),
                               ct3z(cs->ZData[i][j + 1]));
                    glVertex3d(ct3x(cs->XData[0][j + 1]),
                               ct3y(cs->YData[0][i + 1]),
                               ct3z(cs->ZData[i + 1][j + 1]));
        ...
        if (cs->FaceColor != "none") {
            for (int i = 0; i < ny - 1; ++i) {
                for (int j = 0; j < nx - 1; ++j) {
                    ...
                    glBegin(GL_TRIANGLES);
                    glVertex3d(ct3x(cs->XData[i][j]), ct3y(cs->YData[i][j]),
                               ct3z(cs->ZData[i][j]));
        ...
        while (mode < 5) {
            ...
            if ((mode == 1) || (mode == 3)) { // search next contour point
                ...
                while (!is && (m < vc.size())) {
                    is = 0;
                    if ((!vc[m].done) ||
                        ((vc[m].done == 2) && (ac.size() > 2))) {
                        if ((c.xy == 0) && (vc[m].xy == 0) &&
                            (vc[m].xj == c.xj) && (vc[m].yi == c.yi - 1)) {
                            is = 1;
                        }
                        if ((c.xy == 0) && (vc[m].xy == 0) &&
                            (vc[m].xj == c.xj) && (vc[m].yi == c.yi + 1)) {
                        ...
                        if ((c.xy == 0) && (vc[m].xy == 1) &&
                            (vc[m].xj == c.xj) && (vc[m].yi == c.yi)) {
                            is = 3;
                        }
                        if ((c.xy == 0) && (vc[m].xy == 1) &&
                            (vc[m].xj == c.xj + 1) && (vc[m].yi == c.yi)) {
                            is = 4;
                        }
                        if ((c.xy == 0) && (vc[m].xy == 1) &&
                            (vc[m].xj == c.xj) && (vc[m].yi == c.yi - 1)) {
...
dmat MatPlot::peaks(int n)
{
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            ...
            Z[i][j] = a1 / (1 + sr1 / sigma) + a2 / (1 + sr2 / sigma) +
                      a3 / (1 + sr3 / sigma);
        }
    }
    return Z;
...
int MatPlot::bar(dvec x, dvec y) { return bar(x, y, 0.8); }
...
{
    if (is_debug1) {
        printf("display:%4d Patch :type %d, Faces.size %d\n", cp->id, cp->type,
               cp->Faces.size());
    }
    ...
    for (int i = 0; i < nf; ++i) {
        ...
        if (cp->EdgeColor != "none") {
            ...
            for (int iv = 0; iv < nv; ++iv) {
                glVertex2d(ctx(cp->XData[i][iv]), cty(cp->YData[i][iv]));
            }
            glEnd();
        }
        // Face
...
void MatPlot::display_patch_3d()
{
    ...
    for (int i = 0; i < nf; ++i) {
        ...
        if (cp->EdgeColor != "none") {
            ...
            glBegin(GL_LINE_LOOP);
            for (int iv = 0; iv < nv; ++iv) {
                glVertex3d(ct3x(cp->XData[i][iv]), ct3y(cp->YData[i][iv]),
                           ct3z(cp->ZData[i][iv]));
            }
            glEnd();
        }
...
/// Color //
void MatPlot::Shading(string c) { shading(c); }
...
void MatPlot::colormap(vector<vector<float>> c) { cmap = c; }
